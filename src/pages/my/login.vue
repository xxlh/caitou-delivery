<template>
	<view>
		...
		<!-- #ifdef MP-WEIXIN || MP-BAIDU || MP-QQ -->
			<view class="form">
				<view class="subtitle">— 快速登录 —</view>
				<button class="btn" type="btn" @tap="mpGetUserInfo" v-if="callUniLogin === true">一键登录</button>
				<button class="btn" type="btn" :loading="true" v-if="callUniLogin === false">一键登录</button>
			</view>
		<!-- #endif -->
	</view>
</template>
...
<script>
	...
	export default {
		...
		computed: {
            ...mapState(['_token', 'callUniLogin', 'loginProvider'])
		},
		onShow() {
			
		},
		onLoad(query) {
			this.referenceURL = decodeURIComponent(query.referenceURL||'');

			//APP显示第三方登录
			// #ifdef APP-PLUS
			// Todo: 申请微信qq auth key
			// this.isShowOauth=true;
			// this.getProvider();
			// #endif
			
			// #ifdef MP-WEIXIN || MP-BAIDU || MP-QQ
			this.wechatLogin().catch(err => {
				console.log(err);
		...
		methods: {
			...
			mpGetUserInfo() {
				uni.getUserProfile({
					...
					success: (result) => {
						...
						}, {microService: 'user'}).then((userinfoRes) => {
						...
						uni.getStorageInfo({
							success: function (res) {
								res.keys.map((item) => {
									/^\$/.test(item)? uni.removeStorage({
										key: item,
										success: function (res) {
											console.log('removeStorage',item);
										}
									}) :'';
								});
							}
					...
					fail: (result) => {
						uni.showModal({
							title: '登录失败！',
							content: '错误原因' + JSON.stringify(result),
							showCancel: false
						});
					},
				})
			...
			doLogin(){
				...
				post("auth/signin", {
					...
					uni.hideLoading()
					this.setToken(tokenRes.token_type+' '+tokenRes.token);
					this.setUserinfo(tokenRes.userinfo);
					uni.showToast({title: '登录成功',icon:"success"});
